...
function deselectSquad(squad, player)
...
function selectSquad(squad, player)

    -- if there's a currently selected squad, deselect them!
    --DESELECT LOGIC
    deselectSquad(squad, player)

    --Game.print_all(string.format("Squad ID %d selected! Unit in squad: %d", squad.squadID, squad.numMembers) )
...
function handleManualSelectionTool(event, alt)
...
function handleDroidSelectionTool(event, alt)
    local player = game.players[event.player_index]
    local area = event.area;
...
local function handleDroidPickupTool(event, alt)
...
function reportSelectedUnits(event, alt)
	if (event.item == "droid-selection-tool") then
        handleDroidSelectionTool(event, alt)
	elseif (event.item == "droid-pickup-tool") then
        handleDroidPickupTool(event, alt)
    elseif (event.item == "manual-selection-tool") then
        handleManualSelectionTool(event, alt)
	end
end
